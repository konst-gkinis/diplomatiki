...
def s_theory():
    ...
    bx, by, bw, bh = body_area()
    add_image(s, os.path.join(ASSETS, "scope_emissions_infographic.jpeg"),
...
def s_methods():
    ...
    practices = [
        ...
        ("CI/CD", "trunk-based, ~32 deployments/ημέρα"),
...
def s_contributions():
    ...
    cols = [
        ("Τεχνική θεμελίωση", [
            ...
            "Επιτάχυνα κρίσιμα queries με κατάλληλο indexing (GIN, trigrams) και materialized views",
        ]),
        ("Ηγεσία & ομάδα", [
            "Καθοδήγησα και οργάνωσα την ομάδα",
...
def s_results_big():
    ...
    cards = [
        ...
        ("99.999%", "uptime", "χωρίς διακοπή υπηρεσίας"),
...
def build_zoom_appendix():
    ...
    for z in ZOOMS:
        ...
        z["ap"] = ap   # keep a reference so we can mirror speaker notes later
...
→ Ας ξεκινήσουμε από την περιγραφή του προβλήματος.""",
    # 2, Το πρόβλημα: η χειρωνακτική διαδικασία
    """⏱ ~70΄΄
...
(καθυστερήσεις που σήμαιναν κόστος και απώλεια συμβολαίων), ιχνηλασιμότητα και θέμα κλιμάκωσης (η ζήτηση θα ανέβαινε εκθετικά).""",
    # 3, Ο εσωτερικός έλεγχος GIA 2023
    """⏱ ~70΄΄
...
→ Ας δούμε τις απαιτήσεις συμπεριφοράς του συστήματος.""",
    # 5, Μη-λειτουργικές & τεχνικές απαιτήσεις
    """⏱ ~75΄΄
...
LiveView για real-time, και BEAM/Elixir για διαθεσιμότητα.""",
    # 6, Emissions Workbench: τέσσερις πυλώνες
    """⏱ ~80΄΄
...
Δεξιά κάτω, η εξαγωγή σε Excel, που χρηιμοποιείται για ανάλυση δεδομένων από τελικούς χρήστες.""",
    # 10, Energy Bank & Bunker Optimization
    """⏱ ~50΄΄
...
→ Μια σύντομη θεωρητική βάση για τις εκπομπές.""",
    # 11, Λογιστική εκπομπών: τα βασικά
...
→ Ας δούμε ένα χαρακτηριστικό παράδειγμα κώδικα.""",
    # 13, Εκφραστικότητα: Elixir στην πράξη
...
→ Λίγα λόγια για τον τρόπο εργασίας της ομάδας.""",
    # 14, Extreme Programming σε ομάδα 28 μελών
    """⏱ ~70΄΄
...
→ Μερικά συμπεράσματα.""",
    # 17, Μαθήματα & συμβιβασμοί (trade-offs)
    """⏱ ~70΄΄
...
# Mirror each source slide's notes onto its full-screen appendix slide, so the
# notes stay readable while a screenshot is maximized during the defense.
_note_by_slide = dict(zip(prs.slides, NOTES))
for z in ZOOMS:
    _src_note = _note_by_slide.get(z["src"])
    if _src_note is not None:
        z["ap"].notes_slide.notes_text_frame.text = _src_note
